...
public final class Tx3gDecoder extends SimpleSubtitleDecoder {
    ...
    private void decodeInitializationData(List<byte[]> initializationData) {
        if (initializationData != null && initializationData.size() == 1 && (initializationData.get(0).length == 48 || initializationData.get(0).length == 53)) {
            ...
            this.defaultColorRgba = ((initializationBytes[26] & UByte.MAX_VALUE) << 24) | ((initializationBytes[27] & UByte.MAX_VALUE) << 16) | ((initializationBytes[28] & UByte.MAX_VALUE) << 8) | (initializationBytes[29] & UByte.MAX_VALUE);
            ...
            if (this.customVerticalPlacement) {
                int requestedVerticalPlacement = ((initializationBytes[10] & UByte.MAX_VALUE) << 8) | (initializationBytes[11] & UByte.MAX_VALUE);
