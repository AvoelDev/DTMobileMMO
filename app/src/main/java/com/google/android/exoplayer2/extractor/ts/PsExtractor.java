...
public final class PsExtractor implements Extractor {
    ...
    public boolean sniff(ExtractorInput input) throws IOException, InterruptedException {
        ...
        if (PACK_START_CODE == (((scratch[0] & UByte.MAX_VALUE) << 24) | ((scratch[1] & UByte.MAX_VALUE) << 16) | ((scratch[2] & UByte.MAX_VALUE) << 8) | (scratch[3] & UByte.MAX_VALUE)) && (scratch[4] & 196) == 68 && (scratch[6] & 4) == 4 && (scratch[8] & 4) == 4 && (scratch[9] & 1) == 1 && (scratch[12] & 3) == 3) {
            int packStuffingLength = scratch[13] & 7;
            input.advancePeekPosition(packStuffingLength);
            input.peekFully(scratch, 0, 3);
            return 1 == ((((scratch[0] & UByte.MAX_VALUE) << 16) | ((scratch[1] & UByte.MAX_VALUE) << 8)) | (scratch[2] & UByte.MAX_VALUE));
        }
        return false;
    }
    ...
    public int read(ExtractorInput input, PositionHolder seekPosition) throws IOException, InterruptedException {
        ...
        if (nextStartCode == PACK_START_CODE) {
            ...
            if (!this.foundAllTracks) {
                ...
                long maxSearchPosition = (this.foundAudioTrack && this.foundVideoTrack) ? this.lastTrackPosition + 8192 : 1048576L;
